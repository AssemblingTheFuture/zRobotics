...
class System:
  ...
  def plot(self, q = np.array([[]]), delayPerFrame = 200, repeatAnimation = False):
    ...
    def robot(q):
      ...
      self.jointsPositions = q.reshape((n, 1))

      # Computes forward kinematics given current joints' positions
      framesHTM, fkHTM = k.forwardHTM(self, m = m)
    ...
    ani = FuncAnimation(fig, robot, frames = q.T, interval = delayPerFrame, repeat = repeatAnimation)
    plt.show()
