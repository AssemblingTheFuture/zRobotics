...
def trajectory(P, steps, absolut=True, h=0.003):
    ...
    def equation(A, t, grade, h=h):
        ...
        for interval in range(1, int(m / grade)):
            time = np.linspace(t[interval - 1], t[interval],
                               int((t[interval] - t[interval - 1]) / h))
            x = 0
            xd = 0
            xdd = 0
    ...
    def spline(P, t, absolut):
        """
            For «M» matrix
        """
        def pose(t, grade): return [0 if np.isnan(
            t ** i) else t ** i for i in range(grade)]

        def velocity(t, grade): return [0 if np.isnan(
            i * (t ** (i - 1))) else i * (t ** (i - 1)) for i in range(grade)]
        def acceleration(t, grade): return [0 if np.isnan(
            ((i ** 2) - i) * (t ** (i - 2))) else ((i ** 2) - i) * (t ** (i - 2)) for i in range(grade)]
        ...
        for function in range(m):
            ...
            for interval in range(1, n):
                ...
                M[(4 * interval) - 2: (4 * interval) + 2, (grade + 1) * (interval - 1): (grade + 1) * (interval + 1)] = np.array([pose(t[interval], grade=grade + 1) + [0 for i in range(grade + 1)],
                                                                                                                                  [0 for i in range(
                                                                                                                                      grade + 1)] + pose(t[interval], grade=grade + 1),
                                                                                                                                  velocity(
                                                                                                                                      t[interval], grade=grade + 1) + list(-np.array(velocity(t[interval], grade=grade + 1))),
            ...
            v = np.append(v, z if all(value <= 1e-3 for value in abs(M.dot(z) - B))
                          else parameters(M=M, a=v[:, -1].reshape(z.shape), B=B, K=np.eye(4 * (n))), axis=1)
            x, xd, xdd = equation(
                A=v[:, -1].reshape(z.shape), t=t, grade=grade + 1)
            X.append(x)
            Xd.append(xd)
            Xdd.append(xdd)
...
def jointsTrajectory(robot, q0, X, Q, Qd, Qdd, time):
    ...
    for j in range(t.shape[0]):
        ...
        qd = np.linalg.pinv(Jdq).dot(np.eye(8)).dot(
            Q[:, j].reshape((8, 1)) - robot.fkDQ)

        # Dual velocity of each frame
        W = relativeVelocityDQ(
            robot, m=q0.shape[0] + 1, n=q0.shape[0], W0=np.zeros((8, 1)), qd=qd, xi=robot.xi)

        # Time derivative of Dual Jacobian Matrix
        JdDQ = dotJacobianDQ(
            robot, m=q0.shape[0] + 1, W=W[:, 1:], xi=robot.xi, xid=robot.xid)

        # Second derivative of joints' positions
        qdd = np.linalg.pinv(Jdq).dot(Qdd[:, j].reshape((8, 1)) - JdDQ.dot(qd))

        # Solution
        q = np.append(q, dy.solver(f=dy.solver(f=qdd, F=qd),
                      F=q[:, -1].reshape(q0.shape)), axis=1)
    return q, Xr
